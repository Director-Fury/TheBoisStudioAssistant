  ...
  async execute(interaction, config, client) {
    try {
      if (!interaction.member.permissions.has(PermissionFlagsBits.ManageMessages)) {
        return InteractionHelper.safeReply(interaction, { embeds: [errorEmbed('Permission Denied', 'You need **Manage Messages** to pin messages.')], flags: MessageFlags.Ephemeral });
      }
      ...
      await InteractionHelper.safeReply(interaction, { embeds: [successEmbed('Pinned', `Message pinned in ${channel}.`)], flags: MessageFlags.Ephemeral });
      logger.info(`pin-message by ${interaction.user.id} in ${interaction.guildId}`);
    } catch (error) {
      logger.error('pin-message error:', error);
      await handleInteractionError(interaction, error, { commandName: 'pin-message', source: 'pin_message_command' });
    }
  },
};
